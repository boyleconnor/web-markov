    ...
    def add_edge(self, node_one, node_two):
        ...
        if node_one in self.edges:
            if node_two in self.edges[node_one]:
                # Increment weight of edge from node_one to node_two
                self.edges[node_one][node_two] += 1
            else:
                # Create new edge from node_one to node_two of weight 2
                self.edges[node_one][node_two] = 1
        else:
            # Create new adjacency table for node_one, with edge of weight 1 to node_two
            self.edges[node_one] = {node_two: 1}
